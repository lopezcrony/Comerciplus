...
// ----------------------------------------- Descarga de PDF ---------------------------------------------------
  ...
  downloadPDF() {
    ...
    Object.entries(salesByDate).forEach(([date, products]) => {
      ...
      const containerHeightDynamic = Object.keys(products).length * lineHeight * 2 + 20;
      ...
      doc.rect(margin, yPosition, pageWidth - (margin * 2), containerHeightDynamic, 'F');

      // Contenido de la venta
      setStyle(styles.subtitle);
      setColor(colors.primary);
      doc.text(`Ventas del ${date}`, margin + 5, yPosition + lineHeight);

      // Ajuste dinámico de la posición `y`
      let yOffset = yPosition + lineHeight * 2;

      // Iterar sobre los productos vendidos
      Object.entries(products).forEach(([productName, info], index) => {
        // Valores de la columna 1
        setStyle(styles.normal);
        setColor(colors.secondary);
        doc.text(`Producto: ${productName}`, margin + 5, yOffset);
        doc.text(`Cantidad: ${info.cantidad}`, margin + 5, yOffset + lineHeight);

        // Valores de la columna 2
        const col2 = pageWidth / 2 + margin;
        doc.text(`Subtotal: $${info.subtotal.toFixed(2)}`, col2, yOffset);
        setColor(colors.success);
        doc.text(`Total: $${info.subtotal.toFixed(2)}`, col2, yOffset + lineHeight);

        // Incrementar la posición `yOffset` para el siguiente producto
        yOffset += lineHeight * 2;
      });

      yPosition = yOffset + 4; // Ajustar `yPosition` para el siguiente contenedor
    });
